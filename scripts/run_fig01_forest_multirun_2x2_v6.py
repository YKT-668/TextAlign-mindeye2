...
fig.suptitle("CCD@1 on shared982 (pooled, hardneg, K=1)", fontsize=22, x=0.55, y=0.990)
...
for i, subj in enumerate(subj_order):
    ...
    if i in [0, 2]:  # left column panels
        ax.set_yticklabels(d["run_short"].tolist(), fontsize=16)
    ...
    ax.invert_yaxis()
    ax.set_title(panel_title.get(subj, subj), fontsize=20, pad=8)

    # x axis
    ax.set_xlim(xmin, xmax)
    ax.set_xlabel("CCD@1", fontsize=18)

# y label only once (cleaner)
axes[0].set_ylabel("Run (short label)", fontsize=18)
axes[2].set_ylabel("Run (short label)", fontsize=18)
