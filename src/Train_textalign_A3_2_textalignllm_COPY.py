...
hard_neg_path = os.path.join(_PROJ_ROOT, "data/nsd_text/train_coco_captions_hard_negs_clip.pt")
...
if os.path.isfile(teacher_path):
    ...
    print(
        ...
        f"#image_ids={len(pos_image_ids)}, "
        f"min_id={int(pos_image_ids.min())}, max_id={int(pos_image_ids.max())}"
    )
else:
...
if os.path.isfile(hard_neg_path):
    ...
    if not isinstance(state_neg, dict):
        raise RuntimeError(f"[TextAlign] {hard_neg_path} 必须是一个 dict，当前类型: {type(state_neg)}")

    if "image_ids" not in state_neg:
        raise RuntimeError(f"[TextAlign] {hard_neg_path} 缺少 'image_ids' 字段")

    neg_image_ids = state_neg["image_ids"].long()
    # 兼容一下不同 key 命名
    if "neg_text_feats" in state_neg:
        neg_text_feats_teacher = state_neg["neg_text_feats"].to(device)
    elif "text_feats" in state_neg:
        neg_text_feats_teacher = state_neg["text_feats"].to(device)
    else:
        raise RuntimeError(f"[TextAlign] {hard_neg_path} 缺少 'neg_text_feats' 或 'text_feats' 字段")

    id2row_neg = {int(img_id): idx for idx, img_id in enumerate(neg_image_ids.tolist())}

    print(
        f"[TextAlign] HARD-NEG feats shape = {neg_text_feats_teacher.shape}, "
        f"#image_ids={len(neg_image_ids)}, "
        f"min_id={int(neg_image_ids.min())}, max_id={int(neg_image_ids.max())}"
    )

    # 如果正样本也有，检查 image_ids 一致性
    if pos_image_ids is not None:
        if torch.equal(pos_image_ids.cpu(), neg_image_ids.cpu()):
            USE_HARD_NEG = True
            print("[TextAlign] HARD-NEG enabled (image_ids 与正样本完全一致)")
        else:
            print("[TextAlign] WARNING: pos / hard-neg image_ids 不一致，禁用 HARD-NEG")
else:
...
# 默认的缩放系数，后面还会被环境变量覆盖
TEXT_ALIGN_SCALE_DEFAULT = 0.05  # 正样本 InfoNCE
HARD_NEG_EXTRA_SCALE_DEFAULT = 1.0  # 硬负样本额外权重（乘在 InfoNCE 之上）
...
def text_align_hardneg_loss(t_pred, t_pos, t_neg, margin=0.1):
    """
    t_pred: [B, d]   预测文本向量
    t_pos:  [B, d]   正 caption 向量
    t_neg:  [B, d]   硬负 caption 向量（同一图像的“错误描述”）
    目标：cos(t_pred, pos) ≥ cos(t_pred, neg) + margin
    """
    t_pred_n = F.normalize(t_pred, dim=-1)
    t_pos_n  = F.normalize(t_pos,  dim=-1)
    t_neg_n  = F.normalize(t_neg,  dim=-1)

    cos_pos = (t_pred_n * t_pos_n).sum(dim=-1)   # [B]
    cos_neg = (t_pred_n * t_neg_n).sum(dim=-1)   # [B]

    # hinge: max(0, m - cos_pos + cos_neg)
    loss = F.relu(margin - cos_pos + cos_neg).mean()
    return loss
    ...
    for train_i in range(num_iterations_per_epoch):
        with torch.cuda.amp.autocast(dtype=data_type):
           ...
           # ===================== TextAlign: 只在 shared1000 上加文本对齐损失 =====================
            ...
            hard_extra_scale = float(os.environ.get("MINDEYE_TEXTALIGN_HARD_SCALE", str(HARD_NEG_EXTRA_SCALE_DEFAULT)))
            ...
            if use_textalign_flag and alpha_text > 0:
                ...
                if epoch == 0 and train_i < 3 and accelerator.is_main_process:
                    accelerator.print(
                        ...
                        f"use_hard_neg={use_hard_neg_flag}"
                    )
                ...
                if valid_mask.any():
                    ...
                    # 1) batch 内 InfoNCE（正样本）
                    L_pos = text_align_loss(t_pred, t_pos)

                    L_combined = L_pos

                    # 2) 同图像的硬负样本 margin loss（如果可用）
                    if use_hard_neg_flag and (neg_text_feats_teacher is not None) and hard_extra_scale > 0:
                        t_neg = neg_text_feats_teacher[rows_valid]  # [B_valid, 768]
                        L_hard = text_align_hardneg_loss(t_pred, t_pos, t_neg)
