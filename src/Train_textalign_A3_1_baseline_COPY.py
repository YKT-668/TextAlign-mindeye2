...
# ===================== TextAlign: 从 train_coco_text_clip.pt 加载 teacher 文本特征 =====================
import os
import torch

teacher_path = os.path.join(_PROJ_ROOT, "data/nsd_text/train_coco_text_clip.pt")
...
if os.path.isfile(teacher_path):
    ...
    # encode_train_coco_text_clip.py 里保存的是这两个键
    if not isinstance(state, dict):
        raise RuntimeError(f"[TextAlign] {teacher_path} 必须是一个 dict，当前类型: {type(state)}")

    if "image_ids" not in state or "text_feats" not in state:
        raise RuntimeError(f"[TextAlign] {teacher_path} 缺少 'image_ids' 或 'text_feats' 字段")

    image_ids = state["image_ids"].long().tolist()         # 例如 [1910]
    text_feats_teacher = state["text_feats"].to(device)    # [1910, 768]

    # 建立 image_id -> 行号 的映射，用于 batch 里查 teacher 向量
    id2row = {int(img_id): idx for idx, img_id in enumerate(image_ids)}

    print(
        f"[TextAlign] teacher feats shape = {text_feats_teacher.shape}, "
        f"#image_ids={len(image_ids)}, "
        f"min_id={min(image_ids)}, max_id={max(image_ids)}"
    )
else:
    print(f"[TextAlign] teacher file NOT found at {teacher_path}, TextAlign disabled.")

# 只有 teacher 特征和映射都存在，才认为 TextAlign 功能可用
USE_TEXT_ALIGN = (text_feats_teacher is not None) and (id2row is not None)

# 默认的缩放系数，后面还会被环境变量 MINDEYE_TEXTALIGN_SCALE 覆盖
TEXT_ALIGN_SCALE_DEFAULT = 0.05
# =====================================================================
# ===================== TextAlign: 加载 hard-negative 文本特征 =====================
# 默认假设 hard negative 特征和 teacher 一样：
#   - 同样的 image_ids 顺序
#   - 键仍然是 "image_ids" / "text_feats"
hardneg_default_path = os.path.join(
    _PROJ_ROOT, "data/nsd_text/train_coco_text_clip_hardneg.pt"
)
hardneg_path = os.environ.get("MINDEYE_HARDNEG_PATH", hardneg_default_path)
...
if _use_hardneg_flag and text_feats_teacher is not None and id2row is not None:
    if os.path.isfile(hardneg_path):
        ...
        if not isinstance(state_hn, dict):
            raise RuntimeError(
                f"[TextAlign] {hardneg_path} 必须是一个 dict，当前类型: {type(state_hn)}"
            )

        if "image_ids" not in state_hn or "text_feats" not in state_hn:
            raise RuntimeError(
                f"[TextAlign] {hardneg_path} 缺少 'image_ids' 或 'text_feats' 字段"
            )

        # 这里默认 image_ids 顺序与 teacher 完全一致；如果不一致，你之后再单独对齐
        image_ids_hn = state_hn["image_ids"].long().tolist()
        if image_ids_hn != image_ids:
            print(
                "[TextAlign][WARN] hardneg image_ids 与 teacher 不一致，"
                "当前版本假设二者顺序相同，如果需要可以后续显式对齐。"
            )

        text_feats_hardneg = state_hn["text_feats"].to(device)  # [1910, 768]
        print(
            f"[TextAlign] hard-neg feats shape = {text_feats_hardneg.shape}, "
            f"#image_ids={len(image_ids_hn)}"
        )
    else:
        print(
...
# - USE_CF_TEXT_ALIGN: teacher + hardneg 都有，才能用 margin 版本
USE_TEXT_ALIGN = (text_feats_teacher is not None) and (id2row is not None)
USE_CF_TEXT_ALIGN = USE_TEXT_ALIGN and (text_feats_hardneg is not None)
...
# 🔽 新增：挂上文本对齐头（只在 textalign 版里有）
model.text_head = TextAlignHead(
    token_dim=clip_emb_dim,  # 这里 clip_emb_dim 就是 1664
    hidden_dim=2048,
    text_dim=768,            # CLIP-L text 维度
)
utils.count_params(model.backbone)
utils.count_params(model)

_textalign_env_on = os.environ.get("MINDEYE_TEXTALIGN", "1") == "1"

# 在 TextAlign 模式下：冻结检索主干，只训练 text_head。
# 在 baseline 模式下（MINDEYE_TEXTALIGN=0）：训练 ridge+backbone（否则 clip loss 无梯度，会在 backward 时崩溃）。
if _textalign_env_on:
    ...
    for p in model.backbone.parameters():
        p.requires_grad_(True)
    for p in model.text_head.parameters():
        p.requires_grad_(False)
...
if _textalign_env_on:
    ...
    opt_grouped_parameters = [
        {
            'params': [
                p for n, p in model.text_head.named_parameters()
    ...
    # baseline：训练 ridge+backbone（text_head 冻结），只收集 requires_grad=True 的参数
    trainable_named = [(n, p) for n, p in model.named_parameters() if p.requires_grad]
    opt_grouped_parameters = [
        {
            'params': [p for n, p in trainable_named if not any(nd in n for nd in no_decay)],
            'weight_decay': 1e-2,
        },
        {
            'params': [p for n, p in trainable_named if any(nd in n for nd in no_decay)],
            'weight_decay': 0.0,
        },
    ]

optimizer = torch.optim.AdamW(opt_grouped_parameters, lr=max_lr, foreach=False)
...
if multisubject_ckpt is not None:
    load_ckpt("last",outdir=multisubject_ckpt,load_lr=False,load_optimizer=False,load_epoch=False,strict=False,multisubj_loading=False)
...
def text_align_loss_with_cf(
    ...
    """
    t_pred: [B, d]   脑侧预测的文本向量
    t_pos:  [B, d]   正样本 caption 的 CLIP 文本向量
    t_neg:  [B, d]   反事实负样本 caption 的 CLIP 文本向量

    返回:
      L_total: InfoNCE + lambda_cf * margin_loss
    """
    # 先统一归一化（防止数值爆炸，也方便余弦）
    t_pred = F.normalize(t_pred, dim=-1)
    t_pos = F.normalize(t_pos, dim=-1)
    t_neg = F.normalize(t_neg, dim=-1)

    # ① 标准 InfoNCE：让 t_pred 靠近 t_pos，远离同 batch 其他正样本
    logits = t_pred @ t_pos.t() / tau   # [B, B]
    labels = torch.arange(t_pred.size(0), device=t_pred.device)
    L_infonce = F.cross_entropy(logits, labels)

    # ② 反事实 margin：强制 s_pos - s_neg 至少大于 margin
    #    s_pos = cos(t_pred, t_pos)
    #    s_neg = cos(t_pred, t_neg)
    s_pos = (t_pred * t_pos).sum(dim=-1)  # [B]
    s_neg = (t_pred * t_neg).sum(dim=-1)  # [B]
    # 我们希望 (s_pos - s_neg) ≥ margin
    # 所以损失是 ReLU(margin - (s_pos - s_neg))
    L_cf = F.relu(margin - (s_pos - s_neg)).mean()

    L_total = L_infonce + lambda_cf * L_cf
    return L_total
...
for epoch in progress_bar:
    ...
    loss_text_total = 0.  # TextAlign 的 epoch 累积 loss
    ...
    for train_i in range(num_iterations_per_epoch):
        with torch.cuda.amp.autocast(dtype=data_type):
            ...
            # ===================== TextAlign: 只在 shared1000 上加文本对齐损失 =====================
            # ===================== TextAlign: 只在 shared1000 上加文本对齐损失 =====================
            use_textalign_flag = (
                USE_TEXT_ALIGN
                and (len(id2row) > 0)
            ...
            alpha_text = float(os.environ.get("MINDEYE_TEXTALIGN_SCALE", TEXT_ALIGN_SCALE_DEFAULT))
            ...
            if use_textalign_flag and alpha_text > 0:
                ...
                rows = torch.tensor(rows, device=device, dtype=torch.long)  # [B]
                valid_mask = rows >= 0
                ...
                if epoch == 0 and train_i < 3 and accelerator.is_main_process:
                    accelerator.print(
                        ...
                        f"batch_size={len(ids_np)}, shared_hits={int(valid_mask.sum())}, "
                        f"use_cf={USE_CF_TEXT_ALIGN}"
                    )

                # ⭐ 关键改动：如果这个 batch 没有任何 shared1000 的样本，直接跳过本次迭代
                if not valid_mask.any():
                    # 这种 batch 对 text_head 没有监督信号，没必要 backward
                    continue

                rows_valid = rows[valid_mask]                  # [B_valid]
                ...
                if USE_CF_TEXT_ALIGN and (text_feats_hardneg is not None):
                    t_neg = text_feats_hardneg[rows_valid]     # [B_valid, 768]
                    L_text = text_align_loss_with_cf(
                        t_pred,
                        t_pos,
                        t_neg,
                        tau=0.07,
...
# =====================================================================
    ...
    if local_rank==0:
        ...
        if not NEED_IMAGES:
            ...
            _train_den = max(1, (train_i + 1))
            _test_den = max(1, (test_i + 1))
            _train_loss = float(np.mean(losses[-_train_den:])) if len(losses) else float("nan")
            _test_loss = float(np.mean(test_losses[-_test_den:])) if len(test_losses) else float("nan")
            _lr = float(lrs[-1]) if len(lrs) else float(optimizer.param_groups[0]["lr"])

            logs = {"train/loss": _train_loss,
                "test/loss": _test_loss,
                "train/lr": _lr,
                "train/num_steps": len(losses),
                "test/num_steps": len(test_losses),
                "train/fwd_pct_correct": fwd_percent_correct / _train_den,
                "train/bwd_pct_correct": bwd_percent_correct / _train_den,
                "test/test_fwd_pct_correct": test_fwd_percent_correct / _test_den,
                "test/test_bwd_pct_correct": test_bwd_percent_correct / _test_den,
                "train/loss_clip_total": loss_clip_total / _train_den,
                "train/loss_blurry_total": loss_blurry_total / _train_den,
                "train/loss_blurry_cont_total": loss_blurry_cont_total / _train_den,
                "test/loss_clip_total": test_loss_clip_total / _test_den,
                "train/blurry_pixcorr": blurry_pixcorr / _train_den,
                "test/blurry_pixcorr": test_blurry_pixcorr / _test_den,
                "train/recon_cossim": recon_cossim / _train_den,
                "test/recon_cossim": test_recon_cossim / _test_den,
                "train/recon_mse": recon_mse / _train_den,
                "test/recon_mse": test_recon_mse / _test_den,
                "train/loss_prior": loss_prior_total / _train_den,
                "test/loss_prior": test_loss_prior_total / _test_den,
                "train/loss_text": loss_text_total / _train_den,
                }
