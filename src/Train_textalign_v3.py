...
# ===================== TextAlign: teacher & hard-negative text features =====================

# teacher 文本特征（正样本），例如 data/nsd_text/train_coco_text_clip.pt
...
if os.path.isfile(teacher_path):
    ...
    # 全部 COCO image id 列表，例如 [1910, ...]
    image_ids = state["image_ids"].long().tolist()
    # teacher 文本特征 [N, 768]
    text_feats_teacher = state["text_feats"].to(device)

    # 建立 image_id -> 行号 的索引，用于 batch 内查对应的行
...
# 是否启用 TextAlign（有 teacher + id2row 即可）
USE_TEXT_ALIGN = (text_feats_teacher is not None) and (id2row is not None)

# 默认缩放系数（环境变量 MINDEYE_TEXTALIGN_SCALE 会覆盖）
...
if USE_TEXT_ALIGN and os.path.isfile(hardneg_path):
    ...
    # 1) 先处理 image_ids：如果没有，就直接沿用 teacher 的 image_ids
    image_ids_hn = None
    if "image_ids" in state_hn:
        image_ids_hn = state_hn["image_ids"].long().tolist()
    elif image_ids is not None:
        # 直接假设顺序与 teacher 对齐
        image_ids_hn = image_ids
        print(
            "[TextAlign] HARD-NEG 中未找到 image_ids，"
            "默认与 teacher 的 image_ids 顺序一致。"
        )
    else:
        raise RuntimeError(
            f"[TextAlign] {hardneg_path} 中没有 image_ids，且当前也没有 teacher image_ids 可以对齐。"
        )

    # 2) 再寻找文本特征 tensor：优先用 text_feats，没有就自动搜一个形状为 [N,768] 的
    feats = None
    if "text_feats" in state_hn and isinstance(state_hn["text_feats"], torch.Tensor):
        feats = state_hn["text_feats"]
        used_key = "text_feats"
    else:
        # 自动在 dict 里找一个 [N, 768] 的 tensor，当作 hard-neg 文本特征
        used_key = None
        for k, v in state_hn.items():
            if isinstance(v, torch.Tensor) and v.ndim == 2 and v.shape[1] == 768:
                if len(image_ids_hn) == v.shape[0]:
                    feats = v
                    used_key = k
                    break
        if feats is None:
            raise RuntimeError(
                f"[TextAlign] 未在 {hardneg_path} 中找到形如 [N,768] 且 N={len(image_ids_hn)} 的文本特征 tensor，"
                f"可用字段：{list(state_hn.keys())}"
            )
        else:
            print(
                f"[TextAlign] 在 HARD-NEG 中使用字段 '{used_key}' 作为 text_feats "
                f"（shape={feats.shape}）。"
            )

    # 3) 简单检查 image_ids 对齐（如果 teacher 有 image_ids）
    if image_ids is not None and image_ids_hn != image_ids:
        print(
            "[TextAlign][WARN] hardneg image_ids 与 teacher 不一致，"
            "当前版本仍假设顺序匹配，如需严格对齐可后续改成显式索引。"
        )

    text_feats_hardneg = feats.to(device)  # [N, 768]
    print(
        f"[TextAlign] hard-neg feats shape = {text_feats_hardneg.shape}, "
        f"#image_ids={len(image_ids_hn)}, used_key={used_key}"
    )
elif USE_TEXT_ALIGN:
    print(
...
# ===================== TextAlign: 加载 hard-negative 文本特征 END =====================
...
# 冻结大模型，只训练 ridge + text_head 做 TextAlign
...
# clip_img_embedder / autoenc / cnx 前面已经设成 eval + requires_grad_(False) 了

no_decay = ['bias', 'LayerNorm.bias', 'LayerNorm.weight']

opt_grouped_parameters = []

# ① 训练 ridge（被试线性映射），参数量只有几千万，很安全
ridge_decay = [p for n, p in model.ridge.named_parameters()
               if p.requires_grad and not any(nd in n for nd in no_decay)]
ridge_nodecay = [p for n, p in model.ridge.named_parameters()
                 if p.requires_grad and any(nd in n for nd in no_decay)]

if len(ridge_decay) > 0:
    opt_grouped_parameters.append({'params': ridge_decay, 'weight_decay': 1e-2})
if len(ridge_nodecay) > 0:
    opt_grouped_parameters.append({'params': ridge_nodecay, 'weight_decay': 0.0})

# ② 训练 text_head（脑 → 文本 CLIP 向量的小头）
text_decay = [p for n, p in model.text_head.named_parameters()
              if p.requires_grad and not any(nd in n for nd in no_decay)]
text_nodecay = [p for n, p in model.text_head.named_parameters()
                if p.requires_grad and any(nd in n for nd in no_decay)]

if len(text_decay) > 0:
    opt_grouped_parameters.append({'params': text_decay, 'weight_decay': 1e-2})
if len(text_nodecay) > 0:
    opt_grouped_parameters.append({'params': text_nodecay, 'weight_decay': 0.0})

assert len(opt_grouped_parameters) > 0, "没有任何需要训练的参数（检查 requires_grad 设置）"

optimizer = torch.optim.AdamW(opt_grouped_parameters, lr=max_lr)

# 打印一下当前真正在训练的参数量，确认已经大幅缩小
                ...
                if valid_mask.any():
                    ...
                    if USE_CF_TEXT_ALIGN and (text_feats_hardneg is not None):
                        ...
                        L_text = text_align_loss_with_cf(
                            ...
                            t_neg,
                            tau=0.07,
                            margin=0.1,     # 你可以之后做 margin 消融
                            lambda_cf=1.0,  # cf loss 权重（相对 InfoNCE）
                        )
                    else:
